...
function bar() {
    if (pick !== 1) {
        ...
        if ((parseInt(d3.select("tr").style('width'))) * 4 / 5 <= 1140) {
            width = (parseInt(d3.select("tr").style('width')) * 8 / 15) - margin.left - margin.right
            height = (parseInt(d3.select("tr").style('width')) * 32 / 75) - margin.top - margin.bottom
        } else {
            width = (1425 * 8 / 15) - margin.left - margin.right
            height = (1425 * 32 / 75) - margin.top - margin.bottom
        }
        ...
        var margin = {top: 10, right: 30, bottom: 60, left: 40};
            // height = (parseInt(d3.select("tr").style('width')) * 128 / 375) - margin.top - margin.bottom;
        if ((parseInt(d3.select("tr").style('width'))) * 4 / 5 <= 1140) {
            width = (parseInt(d3.select("tr").style('width')) * 32 / 76) - margin.left - margin.right
            height = (parseInt(d3.select("tr").style('width')) * 128 / 375) - 2*margin.bottom
        } else {
            width = (1425 * 32 / 55) - margin.left - margin.right
            height = (1425 * 128 / 375) - margin.top - margin.bottom
        }

        var temp = d3.select("#bar_chart")
            .append("svg")
            .attr("width", parseInt(d3.select("#bar_chart").style('width'))-120)
            .attr("height", height + margin.top + margin.bottom)
            .attr("id", "svg_bar")
        ...
        //     console.log(width)
        // }
        // width = parseInt(d3.select("#bar_chart").style('width'))*4/5
        console.log((width))
        ...
        var z = d3.scaleOrdinal()
            .range(['#4daf4a', '#ff7f00', '#377eb8', '#ff134c']);
        ...
        // console.log(width+34.52, "new one")

        // ### table
        var table = d3.select('#bar_chart')
            .append('table')
            .attr("width", width-40)
            // .attr("width", width+34.52)
            .attr("id", "stats_tb");
